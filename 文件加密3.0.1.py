...
class CryptoWorker(QThread):
        ...
        try:
            ...
            for file_path, file_size in self.queue:
                ...
                try:
                    ...
                    # 针对 10GB 级别载荷优化：避免对超大文件单次完成时频繁强制 GC
                    # 调整为每处理 30 个任务文件或队列结束才统一触发，平衡内存复用率与 I/O 连续性
    ...
    def _enc_v2(self, path, pwd):
        ...
        tmp = out_p + '.tmp'
        fi = None; fo = None  # 提前声明文件句柄以供异常释放
        try:
            salt = os.urandom(16)
            nonce = os.urandom(4)  # 4字节原始随机，配合8字节计数器，严格保持12字节
            kdf = Scrypt(salt=salt, length=32, n=SCRYPT_N_V2, r=8, p=1)
            key = kdf.derive(pwd)
            gcm = AESGCM(key)
            head = MAGIC_V2 + salt + b'\x00\x00\x00\x00' + nonce
            ...
            while self._is_running:
                ...
                if not buf: break
                # 将切块计数器升级为 >Q (8字节)，支持 TB 及无上限级超大文件
            ...
            # 必须在逻辑完整走完且关闭句柄后才执行覆盖，防 10GB 写入中途死锁
            fi.close(); fo.close()
            if self._is_running: 
    ...
    def _dec_v2(self, path, pwd):
        ...
        try:
            ...
            meta = gcm.decrypt(nonce_raw + b'\x00\x00\x00\x00', emeta, MAGIC_V2+salt+nonce_raw)
    ...
    def _enc_v1(self, path, pwd):
        ...
        try:
            ...
            meta = gcm.encrypt(b'\x00\x00\x00\x00' + nonce + b'\x00\x00\x00\x00', struct.pack('>Q', os.path.getsize(path)) + os.path.basename(path).encode('utf-8'), None)
            
            fi = open(path, 'rb')
            fo = open(tmp, 'wb')
            fo.write(MAGIC_V1 + salt + b'\x00\x00\x00\x00' + nonce + struct.pack('>H', len(meta)) + meta)
            c = 1
            while self._is_running:
                buf = fi.read(CHUNK_SIZE)
                if not buf: break
    ...
    def _dec_v1(self, path, pwd):
        ...
        try:
            with open(path, 'rb') as f:
                f.read(5); salt = f.read(16); nonce_raw = f.read(8); nonce = nonce_raw[4:]; mlen_buf = f.read(2)
                if not mlen_buf: return None
                mlen = struct.unpack('>H', mlen_buf)[0]
                emeta = f.read(mlen); pos = f.tell()
            kdf = Scrypt(salt=salt, length=32, n=SCRYPT_N_V1, r=8, p=1); key = kdf.derive(pwd); gcm = AESGCM(key)
            meta = gcm.decrypt(nonce_raw + b'\x00\x00\x00\x00', emeta, None)
            out_name = sanitize_filename(meta[8:].decode('utf-8'))
            out_p = get_safe_win_path(os.path.join(os.path.dirname(path), out_name))
            chk = self._check_overwrite(out_p)
            if not chk: return None
    ...
    def _dec_v0(self, path, pwd):
        ...
        try:
            ...
            while self._is_running:
                ...
                nonce = fi.read(12)
                # 为支持 V0 大文件，将原 c.to_bytes(8) 的安全防护边界与句柄进行对齐
...
class MainWindow(QMainWindow):
    ...
    def _handle_prog(self, inc):
        ...
        # 防止闪断或极高 I/O 硬件下的分母为 0 崩溃
        speed = self.g_proc_bytes / elapsed if elapsed > 0 else 0
        if self.g_total_bytes > 0:
            # 采用整除或精准浮点强转，防止超大长整型乘以 100 时在老版本 Python 环境中由于类型推导产生性能尾差
