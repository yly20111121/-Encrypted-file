...
#BtnUpgrade { background-color: #2e8b57; }
#BtnUpgrade:hover { background-color: #3cb371; }
...
class CryptoWorker(QThread):
    log_sig = Signal(str); progress_sig = Signal(int); file_done_sig = Signal(str, object)
        ...
        try:
            ...
            for file_path, file_size in self.queue:
                ...
                try:
                    ...
                    if res == "ALREADY_V3":
                        self.log_sig.emit(f"[{os.path.basename(file_path)}] 已是最高协议，跳过跃迁。")
                        self.file_done_sig.emit("skip", file_size - self.reported_in_file)
                    elif res:
                        self.consecutive_errors = 0
                        if self.shred and self.mode != 'upgrade': self._secure_delete(file_path)
    ...
    def _process_single(self, path, pwd):
        if self.mode == 'encrypt':
            if self.enc_ver == 3: return self._enc_v3(path, pwd)
            ...
            return self._upgrade_to_v3(path, pwd)
        else:
            with open(path, 'rb') as f: magic = f.read(5)
            if magic == MAGIC_V3: return self._dec_v3(path, pwd)
    ...
    def _enc_v3(self, path, pwd):
        ...
        try:
            salt = os.urandom(16)
            nonce = os.urandom(4) 
            kdf = Scrypt(salt=salt, length=32, n=SCRYPT_N_V3, r=8, p=1)
            key = kdf.derive(pwd)
            gcm = AESGCM(key)
            head = MAGIC_V3 + salt + b'\x00\x00\x00\x00' + nonce
            meta = gcm.encrypt(b'\x00\x00\x00\x00' + nonce + b'\x00\x00\x00\x00', struct.pack('>Q', os.path.getsize(path)) + os.path.basename(path).encode('utf-8'), head)
            ...
            while self._is_running:
                ...
                if not buf: break
                fo.write(struct.pack('>I', len(buf)+16) + gcm.encrypt(nonce + struct.pack('>Q', c), buf, None))
    ...
    def _dec_v3(self, path, pwd):
        tmp = None
        fi = None; fo = None
        try:
            with open(path, 'rb') as f:
                f.read(5); salt = f.read(16); nonce_raw = f.read(8); nonce = nonce_raw[4:]
                mlen_buf = f.read(2)
            ...
            meta = gcm.decrypt(nonce_raw + b'\x00\x00\x00\x00', emeta, MAGIC_V3+salt+nonce_raw)
            ...
            while self._is_running:
                lb = fi.read(4)
                if not lb: break
                chunk = gcm.decrypt(nonce + struct.pack('>Q', c), fi.read(struct.unpack('>I', lb)[0]), None)
    ...
    def _dec_v0(self, path, pwd):
        ...
        try:
            ...
            while self._is_running:
                ...
                nonce = fi.read(12)
                chunk = gcm.decrypt(nonce, fi.read(ln), (c & 0xFFFFFFFFFFFFFFFF).to_bytes(8,'big') + is_last)
                fo.write(chunk); self.progress_sig.emit(len(chunk)); self.reported_in_file += len(chunk); self.heartbeat_sig.emit(); c += 1
    ...
    def _upgrade_to_v3(self, path, pwd):
        tmp = None
        fi = None; fo = None
        try:
            with open(path, 'rb') as f: magic = f.read(5)
            if magic == MAGIC_V3: return "ALREADY_V3"
            
            if magic in (MAGIC_V2, MAGIC_V1):
                # 解析旧元数据
                with open(path, 'rb') as f:
                    f.read(5); old_salt = f.read(16); old_nonce = f.read(8); mlen_buf = f.read(2)
                    old_mlen = struct.unpack('>H', mlen_buf)[0]
                    old_emeta = f.read(old_mlen); pos = f.tell()
                
                old_n = SCRYPT_N_V2 if magic == MAGIC_V2 else SCRYPT_N_V1
                old_kdf = Scrypt(salt=old_salt, length=32, n=old_n, r=8, p=1)
                old_gcm = AESGCM(old_kdf.derive(pwd))
                
                old_aad = MAGIC_V2 + old_salt + old_nonce if magic == MAGIC_V2 else None
                meta_plain = old_gcm.decrypt(old_nonce + b'\x00\x00\x00\x00', old_emeta, old_aad)
                
                # 准备 V3 引擎
                new_salt = os.urandom(16)
                new_nonce_4b = os.urandom(4)
                new_kdf = Scrypt(salt=new_salt, length=32, n=SCRYPT_N_V3, r=8, p=1)
                new_gcm = AESGCM(new_kdf.derive(pwd))
                
                new_head = MAGIC_V3 + new_salt + b'\x00\x00\x00\x00' + new_nonce_4b
                new_emeta = new_gcm.encrypt(b'\x00\x00\x00\x00' + new_nonce_4b + b'\x00\x00\x00\x00', meta_plain, new_head)
                
                tmp = path + '.upgrade.tmp'
                fi = open(path, 'rb')
                fo = open(tmp, 'wb')
                fo.write(new_head + struct.pack('>H', len(new_emeta)) + new_emeta)
                
                fi.seek(pos); c = 1
                while self._is_running:
                    lb = fi.read(4)
                    if not lb: break
                    chunk_plain = old_gcm.decrypt(old_nonce + struct.pack('>I', c), fi.read(struct.unpack('>I', lb)[0]), None)
                    chunk_enc = new_gcm.encrypt(new_nonce_4b + struct.pack('>Q', c), chunk_plain, None)
                    fo.write(struct.pack('>I', len(chunk_plain)+16) + chunk_enc)
                    
                    self.progress_sig.emit(len(chunk_plain)); self.reported_in_file += len(chunk_plain); self.heartbeat_sig.emit(); c += 1
                    
                fi.close(); fo.close()
                if self._is_running: 
                    force_replace(tmp, path)
                    return path
            else:
                self.log_sig.emit(f"[{os.path.basename(path)}] V0 或未识别协议不支持直接流式跃迁，请手动解密后重新封印。")
                return None
                
        finally:
            if 'old_gcm' in locals(): del old_gcm
            if 'new_gcm' in locals(): del new_gcm
            if fi: 
                try: fi.close()
                except: pass
            if fo: 
...
class QueueFilterWorker(QThread):
    results_ready_sig = Signal(list, int, object); finished_sig = Signal()
        ...
        try:
            ...
            for path, sz in self.task_queue:
                ...
                if self.use_smart:
                    try:
                        with open(path, 'rb') as f: magic = f.read(5)
                        if magic in (MAGIC_V3, MAGIC_V2, MAGIC_V1): is_enc = True
                        if magic == MAGIC_V3: is_v3 = True
                    except: pass
                
                # 智能拦截：如果是升级模式且文件已经是V3，或者文件未加密，则直接前置过滤
                if self.use_smart and (
                    (self.mode == 'encrypt' and is_enc) or 
                    (self.mode in ['decrypt', 'upgrade'] and not is_enc) or
                    (self.mode == 'upgrade' and is_v3)
                ): skipped += 1
...
class MainWindow(QMainWindow):
    def __init__(self, initial_files=None):
        super().__init__()
        self.setWindowTitle("AES-GCM Core V3.1.0 协议跃迁版")
        ...
        # 版本映射解耦字典：UI 索引 -> 真实协议版本
        self.ver_map = {0: 3, 1: 2, 2: 1, 3: 0}
        ...
        sets = QHBoxLayout(); self.chk_shred = QCheckBox("🖨️ 彻底粉碎 (仅封印生效)")
        ...
        self.combo_v.addItems(["V3.0.0 (Scrypt N=18) [默认]", "V2.0.2 (Scrypt N=17)", "V1.0.1 (Scrypt N=14)", "V0.0.1 (PBKDF2)"])
        self.combo_v.setCurrentIndex(0)
        main_ly.addWidget(self.combo_v)
        
        act_ly = QHBoxLayout()
        self.btn_enc = QPushButton("开始封印")
        self.btn_dec = QPushButton("解除封印")
        self.btn_upgrade = QPushButton("协议跃迁 (转V3)")
        self.btn_upgrade.setObjectName("BtnUpgrade")
        self.btn_abort = QPushButton("紧急制动 (Abort)")
        self.btn_abort.setEnabled(False)
        act_ly.addWidget(self.btn_enc); act_ly.addWidget(self.btn_dec); act_ly.addWidget(self.btn_upgrade); act_ly.addWidget(self.btn_abort); main_ly.addLayout(act_ly)
        ...
        self.btn_upgrade.clicked.connect(lambda: self._start_batch('upgrade'))
    ...
    def _set_ui_lock(self, locked):
        for b in [self.btn_enc, self.btn_dec, self.btn_upgrade, self.combo_v, self.inp_pwd, self.btn_add, self.btn_dir, self.btn_clear, self.combo_strat]: b.setEnabled(not locked)
    ...
    def _finalize_start_batch(self, filtered, skipped, total_bytes):
        ...
        if pwd:
            if self.current_mode in ('encrypt', 'upgrade'):
                self.active_dialog = QInputDialog(self); self.active_dialog.setWindowTitle("凭据确认"); self.active_dialog.setLabelText("危险操作开始前，再次核对凭据:"); self.active_dialog.setTextEchoMode(QLineEdit.Password)
        ...
        target_ver = self.ver_map[self.combo_v.currentIndex()]
        
        self.g_start_time = time.time()
        self.worker = CryptoWorker(self.current_mode, target_ver, list(self.task_queue), self.vault, self.chk_shred.isChecked(), strat)
    ...
    def _handle_prog(self, inc):
        ...
        self.g_proc_bytes += inc
        elapsed = time.time() - self.g_start_time
        speed = self.g_proc_bytes / elapsed if elapsed > 0 else 0
        if self.g_total_bytes > 0:
            ratio = min(100, max(0, int((self.g_proc_bytes * 100) // self.g_total_bytes)))
            self.prog.setValue(ratio)
    ...
    def _handle_file_done(self, status, remaining):
        ...
        if status in ("skip", "error"):
            self.g_total_bytes -= remaining
            if self.g_total_bytes < 0: self.g_total_bytes = 0
        else:
    ...
    def _set_scan_lock(self, s):
        for b in [self.btn_enc, self.btn_dec, self.btn_upgrade]: b.setEnabled(not s)
